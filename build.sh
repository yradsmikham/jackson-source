...
function git_push() {  
    ...
    echo "GIT PUSH: https://$ACCESS_TOKEN_SECRET@$repo_url --set-upstream origin $BRANCH_NAME"
    git push https://$ACCESS_TOKEN_SECRET@$repo_url --set-upstream origin $BRANCH_NAME
    retVal=$? && [ $retVal -ne 0 ] && exit $retVal
    echo "GIT STATUS"
    git status
}
